...
```{r,fig.height=1.5,fig.cap="ACF/PACF of Differenced data (Seasonal)"}
...
```{r,fig.height=2,fig.cap="SARIMA(4,1,4)(2,1,1)[7] residual plot."}
...
lb_df$`Max Lag` = 365
round(lb_df[,2:4],3)|>kable(col.names = c("LB Statistic","P-value","Max Lag"),caption = "Ljung-Box Test Results")
...
```{r,fig.height=2,fig.cap="SARIMA(3,0,3)(0,1,0)[365] residual plot."}
...
```{r}
#Checking AC in residuals
lb_df=augment(sarima_selected_model)|>features(.resid, ljung_box, lag=30, dof =24)
lb_df$`Max Lag` = 365
round(lb_df[,2:4],3)|>kable(col.names = c("LB Statistic","P-value","Max Lag"),caption = "Ljung-Box Test Results")
...
```{r,fig.height=2,fig.cap="SARIMA(4,1,4)(2,1,1)[7]+Fourier(365) residual plot."}
...
# elec_xts = xts(elec_demand_df$peak_demand,order.by=as.Date(elec_demand_df$Date))
...
```{r}
# spec_garch1=ugarchspec(variance.model=list(model="sGARCH", garchOrder=c(1,1)),mean.model=list(armaOrder=c(7,7),include.mean=T))
# garch1=ugarchfit(spec = spec_garch1, data=log_daily_returns)
...
kable(round(accs,3),caption="Forecasting accuracies of models/methods")
```
