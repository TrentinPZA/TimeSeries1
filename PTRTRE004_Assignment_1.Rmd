...
# Questions/Hypotheses *Need fleshing out*
...
esk_forecasts = elec_demand|>filter(Date>="2023-06-14",Date<="2023-09-14")|>select(Date,fc)
...
### 1.8.1 Time plots of cleaned/transformed data
```{r}
elec_plot=autoplot(elec_demand)+xlab("Time")+ylab("Peak Demand")
gen_plot=autoplot(gen_out)+xlab("Time")+ylab("Energy Loss (UCLF+OCLF)")

p_grid = elec_plot / gen_plot
p_grid
```


### 1.8.2 Data Trend and Seasonality
```{r}
# #Decomp MAYBE JUST PUT IN APPENDIX
...
may_dem=autoplot(may_data,peak_demand)+ylab("Peak Demand")+ggtitle("Daily peak demand May 2021 - May 2023")+xlab("Time")
...
trendseason_grid = (dem_trend/gen_trend)|(may_dem/may_out) 
trendseason_grid
```
...
gg_season(elec_demand)+ylab("Peak Daily Demand") 
...
```{r}
acfd=acf(elec_demand,lag.max=364,plot = F)
{plot(acfd$acf,type="h",xlab="Lag",ylab="ACF")
  abline(h=-0.06,col="blue",lty=2)
  abline(h=0.06,col="blue",lty=2)
  abline(h=0)}
pacfd=pacf(elec_demand,lag.max=364,plot = F)
...
(esk_acc = accuracy(esk_forecasts$fc,test$peak_demand))
```
...
ar_models=train[,-2]|>model(

  `AR(5)`=ARIMA(diffed ~0+pdq(5,0,0)+PDQ(0,0,0)),
  `AR(6)`=ARIMA(diffed ~0+pdq(6,0,0)+PDQ(0,0,0)),
 `AR(7)`=ARIMA(diffed ~0+pdq(7,0,0)+PDQ(0,0,0)),
  `AR(8)`=ARIMA(diffed ~0+pdq(8,0,0)+PDQ(0,0,0))
  )
...
forecasts=unpack_hilo(hilo(forecasts,95),5)
colnames(forecasts)[5:6]=c("lower","upper")

forecast_cumsum=cumsum(forecasts$.mean)
lower_ci_cumsum=cumsum(forecasts$lower)
upper_ci_cumsum=cumsum(forecasts$upper)


final_obs=tail(elec_demand$peak_demand, 1)


backtransformed_forecasts=forecast_cumsum+final_obs
backtransformed_lower_ci=lower_ci_cumsum+final_obs
backtransformed_upper_ci=upper_ci_cumsum+final_obs

ar_acc=accuracy(backtransformed_forecasts,test$peak_demand)
#
ar_backtransformed_forecasts= data.frame(Date=forecasts$Date,
                                           Forecast=backtransformed_forecasts,
                                           Lower=backtransformed_lower_ci,
                                           Upper=backtransformed_upper_ci)|> as_tsibble(index=Date)

ggplot() +
  geom_line(data=train, aes(x=Date, y=peak_demand), color='black') +
  geom_line(data=ar_backtransformed_forecasts, aes(x=Date, y=Forecast), color='blue') +
#  geom_ribbon(data = ar_backtransformed_forecasts, aes(x = Date, ymin = Lower, ymax = Upper), alpha = 0.2)+
  labs(title="92 Day Forecast for AR(8)",
       x="Time",
       y="Peak Demand") +
  theme_minimal()
```
...
```{r}
ma_models=train[,-2]|>model(
  `MA(4)`=ARIMA(diffed ~0+pdq(0,0,4)+PDQ(0,0,0)),
  `MA(5)`=ARIMA(diffed ~0+pdq(0,0,5)+PDQ(0,0,0)),
  `MA(6)`=ARIMA(diffed ~0+pdq(0,0,6)+PDQ(0,0,0)),
  `MA(7)`=ARIMA(diffed ~0+pdq(0,0,7)+PDQ(0,0,0))
...
#
#
ma_acc=accuracy(backtransformed_forecasts,test$peak_demand)
ma_backtransformed_forecasts= data.frame(Date=forecasts$Date,
                                           Forecast=backtransformed_forecasts,
                                           Lower=backtransformed_lower_ci,
                                           Upper=backtransformed_upper_ci)|> as_tsibble(index=Date)
...
ggplot() +
  geom_line(data=train, aes(x=Date, y=peak_demand), color='black') +
  geom_line(data=ma_backtransformed_forecasts, aes(x=Date, y=Forecast), color='blue') +
 #geom_ribbon(data = ma_backtransformed_forecasts, aes(x = Date, ymin = Lower, ymax = Upper), alpha = 0.2)+
  labs(title="92 Day Forecast for MA(7)",
       x="Time",
       y="Peak Demand") +
  theme_minimal()
```
...
```{r}
#ARMA Models
arma_models= train[,-2]|>model(
  `ARMA(4,4)`=ARIMA(diffed ~0+pdq(4,0,4)+PDQ(0,0,0)),
  `ARMA(4,5)`=ARIMA(diffed ~0+pdq(4,0,5)+PDQ(0,0,0)),
  `ARMA(5,4)`=ARIMA(diffed ~0+pdq(5,0,4)+PDQ(0,0,0)),
  `ARMA(5,5)`=ARIMA(diffed ~0+pdq(5,0,5)+PDQ(0,0,0)),
  `ARMA(5,6)`=ARIMA(diffed ~0+pdq(5,0,6)+PDQ(0,0,0)),
  `ARMA(5,7)`=ARIMA(diffed ~0+pdq(5,0,7)+PDQ(0,0,0))
...
arma_acc=accuracy(backtransformed_forecasts,test$peak_demand)
backtransformed_forecasts= data.frame(Date=forecasts$Date,
                                           Forecast=backtransformed_forecasts,
                                           Lower=backtransformed_lower_ci,
                                           Upper=backtransformed_upper_ci)|> as_tsibble(index=Date)
...
ggplot() +
  geom_line(data=train, aes(x=Date, y=peak_demand), color='black') +
  geom_line(data=backtransformed_forecasts, aes(x=Date, y=Forecast), color='blue') +
 # geom_ribbon(data = backtransformed_forecasts, aes(x = Date, ymin = Lower, ymax = Upper), alpha = 0.2)+
  labs(title="92 Day Forecast for ARMA(5,6)",
       x="Time",
       y="Peak Demand") +
  theme_minimal()
```
...
# save(sarima_selected_model, file = "sar_fourier.RData")
# acfd=pacf(residuals(sarima_selected_model),lag.max = 365,plot = F)
# {plot(acfd$acf,type="h",xlab="Lag",ylab="ACF")
#   abline(h=-0.06,col="blue",lty=2)
#   abline(h=0.06,col="blue",lty=2)
#   abline(h=0)}
...
## 2.8 Comparing models and residual analysis
...
```{r}
# autoplot(elec_demand,peak_demand)+autolayer(esk_forecasts,fc,color="red")
```
