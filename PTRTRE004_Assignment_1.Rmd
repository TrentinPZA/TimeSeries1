...
gen_trend=gen_out |>model(STL(unplanned_outages~trend(window=91)))|>
  components() |>
...
The log returns of the data are calculated as such:
...
The log returns are then also squared, this further exposes clusters of volatility and ensures that the negative and positive log returns contribute equally to the variance that is being modeled. Squaring the log returns also amplifies the larger shocks in the log returns which allow the GARCH model to identify the periods of high volatility.

```{r,fig.height=3,fig.cap="Time and ACF plots of returns/squared returns."}
...
log_returns_acf=autoplot(acf((log_daily_returns^2), plot = FALSE)) +
  labs(title = "Squared Log Daily Returns",
       x = "Lag",
       y = "ACF") +
    geom_hline(yintercept = 0, linetype = "solid", color = "black") +
  theme_minimal()

log_returns_squared_acf=autoplot(pacf((log_daily_returns^2), plot = FALSE)) +
  labs(title = "Squared Log Daily Returns",
       x = "Lag",
       y = "PACF") +
    geom_hline(yintercept = 0, linetype = "solid", color = "black") +
...
The $ARCH$ models were fitted with an $ARMA$ mean component giving us $ARCH(q)-ARMA(p,q)$ which were fitted to the returns data. The orders for the $ARMA(p,q)$ component were selected using the ACF of the squared log returns.

For the mean component of the first model $p=1$ and $q=1$ were chosen due to the significant spike in autocorrelation at lag 1. For the variance component $q=1$ was chosen. 

The significant autocorrelation at lag 7 and multiples of 7 is due to the strong weekly seasonality in the series, to try and capture this autocorrelation an $ARCH(1)-ARMA(7,7)$ was fitted. An $ARCH(2)-ARMA(7,7)$ was then also fitted to try and capture the autocorrelation present at higher lags. 

```{r}
spec_arch1=ugarchspec(variance.model=list(model="sGARCH", garchOrder=c(1,0)),mean.model=list(armaOrder=c(1,1),include.mean=T))
arch1=ugarchfit(spec = spec_arch1, data=log_daily_returns)

spec_arch2=ugarchspec(variance.model=list(model="sGARCH", garchOrder=c(1,0)),mean.model=list(armaOrder=c(7,7),include.mean=T))
arch2=ugarchfit(spec = spec_arch2, data=log_daily_returns)

spec_arch3=ugarchspec(variance.model=list(model="sGARCH", garchOrder=c(2,0)),mean.model=list(armaOrder=c(7,7),include.mean=T))
...
The residuals and model fits in the figure above show that the $ARCH(1)-ARMA(1,1)$ severely under fits the series. This results in significant autocorrelation in the standardized residuals at low lags such as lag 3 as well as poor model fit as shown by the bands around the log return series which fail to capture most of the volatility in the series.

The $ARCH(1)-ARMA(7,7)$ model provides and improvement on the first model, with significant autocorrelation in the residuals only being present at higher lags. The model fit also shows improvement as the bands surrounding the series somewhat mimic the volatility in the series.

The model fit provided by the $ARCH(2)-ARMA(7,7)$ appears to best capture the volatility in the series however it is still not perfect and there is still significant autocorrelation present in the residuals which suggests the model is not capturing all available information.

### 2.6.2 - GARCH

Like the $ARCH$ models, the $GARCH$ models were fitted with an $ARMA$ mean component which gives us $GARCH(p,q)-ARMA(p,q)$ models. The $ARCH(1)-ARMA(7,7)$ model and the $ARCH(7)-ARMA(7,7)$ model were then modified to include the past variance component $p$ as they provided an indication of what models are better suited to modelling the series.

Therefore the models fitted were the $GARCH(1,1)-ARMA(7,7)$, $GARCH(2,1)-ARMA(7,7)$ and $GARCH(2,2)-ARMA(7,7)$.
...
The figure above shows that the inclusion of the past variance component helped with modelling the autocorrelation present in the returns as there is no significant autocorrelation in any of the three models. The model fit for all three models is decent but show that the models do not accurately capture the volatility in the data.

## 2.7 - Comparing Forecasting models/methods

### 2.7.1 - Mean Forecasting Models
...
The models fit in Section 2.6 exhibited either significant autocorrelation in the residuals, poor model fit (by failing to accurately capture the volatility) or both. This is likely due to the use of an $ARMA(p,q)$ for the mean component of the ARCH/GARCH models which does not account for the seasonality. Therefore, none of the ARCH/GARCH models fitted are appropriate for the capturing the volatility in the data. I would recommend either seasonally adjusting the data or alternative models for the mean component such as a SARIMA model which explicitly incorporates the seasonality present and thus, is much more likely to provide an appropriate fit to the data.

# Section 3 - Total UCLF+OCLF (Energy unavailability) Model Fitting

## Differencing for Non-seasonal Models
```{r}
gen_train = gen_out|>filter(Date<="2023-06-14")
gen_test = gen_out|>filter(Date>="2023-06-14")
```

```{r,fig.cap="Differenced energy unavailability data and ACF/PACF plots (Non-seasonal)."}
kpss.test(gen_out$unplanned_outages,null = "Trend",lshort = F)
...
gen_detrended|>autoplot()
colnames(gen_detrended)=c("Date","unplanned_outages")
acf(gen_detrended$unplanned_outages)
pacf(gen_detrended$unplanned_outages)
gg_tsdisplay(gen_detrended,unplanned_outages, plot_type='partial')

# gen_out$diffed=difference((gen_out$unplanned_outages),lag=1,differences=1) 
# gg_tsdisplay(gen_out,diffed, plot_type='partial')
```


```{r}
arima_models=gen_train|>model(`AR(1)`=ARIMA(unplanned_outages ~1+pdq(3,0,0)+PDQ(0,0,0)),
                              `ARIMA(1,1,1)`=ARIMA(unplanned_outages ~1+pdq(3,1,4)+PDQ(0,0,0)))

arima_selected_model=arima_models |> select(`AR(1)`)
arima_selected_model=arima_models |> select(`ARIMA(1,1,1)`)
arima_selected_model|>forecast(h="3 months")|>autoplot(gen_train)
fcfc=arima_selected_model|>forecast(h="92 days")
accuracy(fcfc$.mean,gen_test$unplanned_outages)
...
```{r}
fcfc$forecast= fcfc$.mean + trend_forecast$.mean
fcfc= unpack_hilo(hilo(fcfc, 95))


fcfc$lower= as.numeric(fcfc$`95%`$lower) + as.numeric(trend_forecast$.mean)
fcfc$upper= as.numeric(fcfc$`95%`$upper) + as.numeric(trend_forecast$.mean)
...
ggplot(fcfc) +
  ...
  labs(title = "Forecast with Prediction Intervals", x = "Date", y = "Value") +
  theme_minimal()


accuracy(fcfc$forecast,gen_test$unplanned_outages)
```
