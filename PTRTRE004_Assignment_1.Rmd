...
acf(gen_out$unplanned_outages)
pacf(gen_out$unplanned_outages)
```
...
```{r}
filted = gen_out[,-3]|>filter(Date>="2023-08-01")
auto.arima(gen_out$diffed,seasonal = F)
arima_models=gen_train|>model(MAIN=ARIMA(unplanned_outages ~1+pdq(1,0,0)+PDQ(0,0,0)))
arima_selected_model=arima_models |> select("MAIN")
...
```{r}
naurmodel=auto.arima(gen_out[,-3],seasonal = F)

naurmodel|>forecast()|>autoplot()
```
